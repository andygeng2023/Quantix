...
function quantix_series_metrics(array $prices):array{
 ...
 $path=[];for($d=1;$d<=20;$d++){ $vals=[];$weights=[];foreach($models as $m){$vals[]=exp($m['int']+$m['slope']*($m['n']-1+$d));$weights[]=1/($m['sigma']*$m['sigma']);}$sw=array_sum($weights);$v=0;foreach($vals as $i=>$vv)$v+=($weights[$i]/$sw)*$vv;$variance=0;foreach($models as $i=>$m)$variance+=($weights[$i]/$sw)*(($m['sigma']**2)+(($vals[$i]-$v)**2))/(1+$d/$m['n']);$spread=1.96*sqrt(max($variance,1e-12));$path[]=['day'=>$d,'value'=>$v,'low'=>exp(log($v)-$spread),'high'=>exp(log($v)+$spread)];}
