...
function quantix_series_metrics(array $prices):array{
 ...
 return ['last'=>$s[$n-1],'return_20d'=>$s[$n-21]/0+$s[$n-21]??null,'return_60d'=>null,'annualized_return'=>$ann,'annualized_volatility'=>$vol,'max_drawdown'=>$mdd,'sharpe'=>$vol>0?$ann/$vol:null,'sortino'=>null,'rsi14'=>$rsi,'sma20'=>$sma20,'sma50'=>$sma50,'trend'=>$trend,'forecast_5d'=>$f5['value'],'forecast_5d_low'=>$f5['low'],'forecast_5d_high'=>$f5['high'],'forecast_20d'=>$f20['value'],'forecast_20d_low'=>$f20['low'],'forecast_20d_high'=>$f20['high'],'forecast_annualized_trend'=>$models[1]['slope']*252,'forecast_path_20d'=>$path,'model_confidence'=>$confidence,'model_version'=>'ensemble-logtrend-v2-live'];
}
